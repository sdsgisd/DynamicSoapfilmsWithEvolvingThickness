...
void HGF::meshAsMatrices(Eigen::MatrixXd& V, Eigen::MatrixXi& F, bool withoutGhostGeometry){
    using namespace Eigen;
    
    if(withoutGhostGeometry){
        
        V.setZero(real_vertices.size(), 3);
        F.setZero(real_triangles.size(), 3);
        
        for(size_t vii=0;vii<real_vertices.size();++vii){
            V.row(vii)=pos(real_vertices[vii]);
        }
        for( size_t tii = 0; tii < real_triangles.size(); ++tii ){
            const auto& tri=mesh().m_tris[real_triangles[tii]];
            F.row(tii)<<verts_to_real[tri[0]],verts_to_real[tri[1]],verts_to_real[tri[2]];
...
void HGF::writeMesh_FaceLabel_constrainedVertices(const bool with_imaginary_vertices){
    
    // with_normal is absolutely needed for t-junction vertices.
    // This assigns region-wise vertex normals and reduce rendering artifacts.
    
    if(saveMeshExtension==".obj"){
        MeshIO::saveOBJ(*this, "mesh.obj",with_imaginary_vertices,saveMeshWith_normal,saveMeshChange_y_z);
    }else if (saveMeshExtension==".ply"){
        MeshIO::savePLY(*this, "mesh.ply",with_imaginary_vertices,saveMeshChange_y_z);
    }else{
        std::cout<<"An invalid extension of mesh detected in HGF::writeMesh_FaceLabel_constrainedVertices. Please specify either of '.obj' or '.ply'. Finish simulation.\n";
        exit(-1);
    }
...
void HGF::write_film_mesh(std::string outputfile){
    ...
    if(saveMeshExtension==".obj"){
        MeshIO::saveOBJ(*this, outputfile,with_imaginary_vertices,saveMeshWith_normal,saveMeshChange_y_z);
    }else if (saveMeshExtension==".ply"){
        MeshIO::savePLY(*this, outputfile,with_imaginary_vertices,saveMeshChange_y_z);
    }else{
        std::cout<<"An invalid extension of mesh detected in HGF::write_film_mesh. Please specify either of '.obj' or '.ply'. Finish simulation.\n";
        exit(-1);
    }
    
}

void HGF::write_mesh(const std::string& output_directory){
    static int count=0;
    
    std::stringstream output_meshfile;
    output_meshfile<<output_directory<<"output_mesh"<<std::setfill('0') << std::setw(6)<<count<<saveMeshExtension;
...
void HGF::geometric_information(double time){
    ...
    const double bottomPosValue=V.col(2).minCoeff();
    std::cout<<"bottomPosValue:"<<bottomPosValue<<std::endl;
...
void HGF::compute_vertex_areas(std::vector<double> & v_areas){
    
    size_t nv=mesh().nv();
	v_areas = std::vector<double>( nv, 0.0 );
    
    size_t nt=mesh().nt();
    for (size_t t = 0; t < nt; t++)
    {
        double tri_area=surfTrack()->get_triangle_area(t);
        const auto& tri=mesh().get_triangle(t);

        for(size_t v=0;v<3;++v){
            v_areas[tri[v]]+=1.0/3.0*tri_area;
        }
    }
...
void HGF::volumes_and_areas_sparse(const Eigen::MatrixXd &targetU , Eigen::VectorXd &volumes, Eigen::SparseMatrix<double>& area_matrix ){
    ...
    volumes=VectorXd::Zero(num_initialBubbles);
    area_matrix.resize(num_initialBubbles,num_initialBubbles);
    ...
    for (size_t t = 0; t < nt; t++)
    {
        ...
        // total volume via divergence theorem.
    ...
    for(int i=0;i<num_initialBubbles;++i){
...
void HGF::volumes_and_areas(const Eigen::MatrixXd &targetU , Eigen::VectorXd &volumes, Eigen::MatrixXd& area_matrix){
    ...
    volumes=VectorXd::Zero(num_initialBubbles);
    area_matrix=MatrixXd::Zero(num_initialBubbles,num_initialBubbles);
...
#ifdef FEOS
void HGF::writeVelocityOfPoint( double time, bool write )
{
	using namespace Eigen;
	
	Vector3d velocity;
	static size_t vertex_ind = 100;

    velocity = fv( vertex_ind );
	

	static std::string directory = "./";
	static std::string filename = "velocityOfPoint.csv";

	std::cout << "velocity:" << velocity[ 0 ] << "," << velocity[ 1 ] << "," << velocity[ 2 ] <<","<<velocity.norm()<< std::endl;
	...
	if( write ){
		...
		output << time << "," << velocity[0]<<","<< velocity[ 1 ] <<","<< velocity[ 2]<<velocity.norm()<<std::endl;
...
void HGF::nonManifoldJunctions(std::vector<size_t>& t_junctions,std::vector<size_t>& q_junctions){
    ...
    for (size_t v = 0; v < num_v; ++v) {
        if (not mesh().is_vertex_nonmanifold(v)) {
            continue;
        }
        ...
        for (size_t ei = 0; ei < num_incident_edges; ++ei) {
            size_t edge_ind = incident_edges[ei];
