...
//    // FIXME: temporarily swaped y and z axis to vertically stand the double bubble.
//    for(size_t vi=0;vi<vs.size();++vi){
//        std::swap(vs[vi][0],vs[vi][2]);
//    }
...
HGF * Scenes::sceneFoamInit(Sim * sim, std::vector<LosTopos::Vec3d> & vs, std::vector<LosTopos::Vec3st> & fs, std::vector<LosTopos::Vec2i> & ls, std::vector<size_t> & cv, std::vector<Vec3d> & cx)
{
    ...
    if(sub_scene=="triple"){
        ...
        spheres[1].second = 0.5;
    }else  if(sub_scene=="3squarepyramid"){
        const double radius = 0.5;
        const double eradius = radius-0.01;
        const int num_levels = 3;
        const int num_cells = (num_levels*(num_levels+1)*(2*num_levels+1))/6;
        
        int bubble_counter = 0;
        spheres.resize(num_cells);
        for (size_t i=0; i<num_levels; i++) {
            for (size_t j=0; j<num_levels-i; j++) {
                for (size_t k=0; k<num_levels-i; k++) {
                    spheres[bubble_counter].first = Vec3d(i*radius+j,i*radius+k,i*0.5*std::sqrt(2));
                    spheres[bubble_counter].second = eradius;
                    bubble_counter++;
                }
            }
        }
    }else  if(sub_scene=="column"){
        const double radius = 0.5;
        const double eradius = radius-0.01;
        const int num_levels = 3;
        
        spheres.resize(num_levels);
        for (size_t i=0; i<num_levels; i++) {
            spheres[i].first = Vec3d(0,0,i);
            spheres[i].second = eradius;
        }
    }else{
...
HGF * Scenes::sceneBubbleWand(Sim * sim, std::vector<LosTopos::Vec3d> & vs, std::vector<LosTopos::Vec3st> & fs, std::vector<LosTopos::Vec2i> & ls, std::vector<size_t> & cv, std::vector<Vec3d> & cx)
{
    ...
    int num_initialBubbles=1;//0;
    
    return new HGF(vs, fs, ls, cv, cx,num_initialBubbles);
}
...
HGF * Scenes::sceneTwoRingsPinching(Sim * sim, std::vector<LosTopos::Vec3d> & vs, std::vector<LosTopos::Vec3st> & fs, std::vector<LosTopos::Vec2i> & ls, std::vector<size_t> & cv, std::vector<Vec3d> & cx)
{
    ...
    bool cmc=false;
    int num_initialBubbles=cmc?1:0;

    HGF * hgf=new HGF(vs, fs, ls, cv, cx,num_initialBubbles);
...
HGF * Scenes::sceneLattice(Sim *sim, std::vector<LosTopos::Vec3d> &vs, std::vector<LosTopos::Vec3st> &fs, std::vector<LosTopos::Vec2i> &ls, std::vector<size_t> &cv, std::vector<Vec3d> &cx){
    ...
    int num_initialBubbles=(array_z-1)*(array_y-1)*(array_x-1);
    return new HGF(vs, fs, ls, cv, cx);

}

HGF * Scenes::sceneSphereOverFilm(Sim * sim, std::vector<LosTopos::Vec3d> & vs, std::vector<LosTopos::Vec3st> & fs, std::vector<LosTopos::Vec2i> & ls, std::vector<size_t> & cv, std::vector<Vec3d> & cx)
{
    ...
    // TODO: Temporary comment out for spanning film.
    ...
    int num_initialBubbles=centers.size();
...
HGF * Scenes::sceneSquareFilm(Sim * sim, std::vector<LosTopos::Vec3d> & vs, std::vector<LosTopos::Vec3st> & fs, std::vector<LosTopos::Vec2i> & ls, std::vector<size_t> & cv, std::vector<Vec3d> & cx)
{
    ...
    vs.resize(v_film.size());
    fs.resize(f_film.size());
    ls.resize(l_film.size());
    
    for (size_t i = 0; i < v_film.size(); i++){
        vs[i]=vc(v_film[i]);
    }
    for (size_t i = 0; i < f_film.size(); i++){
        fs[i]=LosTopos::Vec3st(f_film[i][0], f_film[i][1], f_film[i][2]);
    }
    for (size_t i = 0; i < l_film.size(); i++){
        ls[i]=LosTopos::Vec2i(l_film[i][0], l_film[i][1]);
    }
    ...
    const std::string direction=Options::strValue("direction");
    if (!direction.compare("vertical")) {
        for (size_t i = 0; i < vs.size(); i++){
            std::swap(vs[i][1],vs[i][2]);
        }
        for (size_t i = 0; i < cv.size(); i++)
        {
            std::swap(cx[i][1],cx[i][2]);

        }
    }
    
    HGF * hgf=new HGF(vs, fs, ls, cv, cx,0);
    Scenes::square_frame(sim,hgf,out,height);
    return hgf;
    
}

HGF * Scenes::sceneTwistedFilm90(Sim * sim, std::vector<LosTopos::Vec3d> & vs, std::vector<LosTopos::Vec3st> & fs, std::vector<LosTopos::Vec2i> & ls, std::vector<size_t> & cv, std::vector<Vec3d> & cx)
{
    ...
    // "bottom" vertices on the x=0 line
    v_film.emplace_back(0,-out,height);
    v_film.emplace_back(0,out,height);
    // "top" vertices (rotated by pi/2) on the y=0 line
    v_film.emplace_back(-out,0,-height);
    v_film.emplace_back(out,0,-height);
    // ghost vertices for Los Topos
    v_film.emplace_back(-2*out,-2*out,0);
    // center vertex for remeshing
    v_film.emplace_back(0,0,0);
    
    // insert actual triangles
    for(int i=0;i<4;++i){
        f_film.emplace_back(i,(i+1)%4,4);
        l_film.emplace_back(1,0);
    ...
    // determine constrained vertices and positions
    for(int i=0;i<5;++i){
        cv.push_back(i);
    }
    for (size_t i = 0; i < cv.size(); i++)
...
void Scenes::stepFoamInit(double dt, Sim * sim, HGF * hgf)
{
    ...
    const bool growBubblesViaNormal=false;
    ...
    if(growBubblesViaNormal){
        ...
        int num_initialBubbles=hgf->num_initialBubbles;
...
int Scenes::burstOneBubble(double dt, Sim * sim, HGF * hgf, int target_region){
    
    std::cout << "Bursting a bubble now." << std::endl;
    ...
    if(target_region==-1){
        ...
        for (size_t i = 0; i < hgf->mesh().nt(); i++)
        {
            ...
            if (l[0] > hgf->AIR and l[1] == hgf->AIR)
                burstable_regions_set.insert(l[0]);
        ...
        if(burstable_regions.empty()){
            std::cout << "No eligible regions to burst\n";
            return -1;
        }
        region_to_burst = burstable_regions[rand() % burstable_regions.size()];
    }else{
        region_to_burst=target_region;
    }
    
    std::cout << "Region to be bursted: " << region_to_burst << std::endl;
    
    if(hgf->absorbLiquidAftuerBurstCoef>0.0){
        hgf->absorbLiquidFromABubble(region_to_burst);
    }
    ...
    --hgf->num_currentBubbles;
    std::cout << "Bubble bursted." << std::endl;
    std::cout<<"num of remaining bubbles is "<<hgf->num_currentBubbles<<".\n";

    return region_to_burst;
    
}

int Scenes::burstOneThinBubble(double dt, Sim * sim, HGF * hgf){
    
    static double s_next_burst = 0.0;
    ...
    const size_t num_t= hgf->mesh().nt();
    
    bool burstBubble=false;
    int targetRegion=-1;
    for (size_t ti : hgf->real_triangles)
    {
        LosTopos::Vec2i l = hgf->mesh().get_triangle_label(ti);
        if(l[0] <  l[1]){
            std::swap(l[0],l[1]);
        }
        //if (l[0] == 0 and l[1] == 0)
        if (l[0] > hgf->AIR and l[1] == hgf->AIR){
            if(ThTri[ti]<=hgf->burst_threshold){
                std::cout<<ThTri[ti]<<","<<hgf->burst_threshold<<"\n";
                targetRegion=l[0];
                burstBubble=true;
                break;
            }
    ...
    if(burstBubble){
        ...
        s_next_burst= sim->m_time +Options::doubleValue("auto-burst-interval");
        
        // Correct non-manifold vertices that are incident to the region to pop.
        // i.e. incident to a triangle with the lavel (region,air)
        //
        std::unordered_map<int,std::set<size_t>> region2boundaryVertices;
        for (size_t ti : hgf->real_triangles)
        {
            LosTopos::Vec2i l = hgf->mesh().get_triangle_label(ti);
            if(l[0] <  l[1]){
            ...
            int remainingRegion=-1;
            if ( l[0] == targetRegion and l[1] > hgf->AIR ){
                remainingRegion=l[1];
            }else if(l[0] > hgf->AIR and l[1] == targetRegion){
                remainingRegion=l[0];
            }else{
                continue;
            }
            ...
            for(size_t vi_t=0;vi_t<3;++vi_t){
                const size_t& vert=hgf->mesh().m_tris[ti][vi_t];
                if(hgf->mesh().is_vertex_nonmanifold(vert)){
                    region2boundaryVertices[remainingRegion].insert(vert);
                }
            }
        }
        ...
        // i.e. sum of liquid on triangles with label (region,air)
        ...
        for (size_t ti : hgf->real_triangles)
        {
            ...
            if (l[0] == targetRegion and l[1] == hgf->AIR){
                lostLiquidVolume+=ThTri[ti]*hgf->triangle_areas[ti];
            }
        }
        return burstOneBubble(dt, sim, hgf,targetRegion);
...
int Scenes::burstThinnestBubble(double dt, Sim * sim, HGF * hgf)
{
    using namespace Eigen;
    VectorXd ThTri;
    hgf->scalarVert2triNaive(hgf->thvv(), ThTri);
    ...
    Eigen::VectorXd LowestThinnesse=VectorXd::Constant(hgf->num_region, LargeValue);
    ...
    for (size_t i = 0; i < hgf->mesh().nt(); i++)
    {
        ...
        //if (l[0] == 0|| l[1] == 0)
        if (l[0] > hgf->AIR and l[1] == hgf->AIR){
			// This blacket of std::min is for windows.h header
            LowestThinnesse[l[0]]=(std::min)(LowestThinnesse[l[0]],ThTri[i]);
        }
    }
    
    int targetRegion=-1;
    const double minThinness=LowestThinnesse.minCoeff(&targetRegion);
    if(minThinness<LargeValue){
        std::cout<<"burst region "<<targetRegion<<" with lowest thickness "<<minThinness<<"\n";
...
int Scenes::burstBubblesRandomly(double dt, Sim * sim, HGF * hgf)
{
    ...
    if (sim->m_time >= s_next_burst)
    {
        ...
        return Scenes::burstOneBubble(dt, sim, hgf);
    }
...
void Scenes::stepBubbleWand(double dt, Sim * sim, HGF * hgf)
{
    ...
    const double move_velocity=-1; wand_velocity;
...
void Scenes::stepSphereOverFilm(double dt, Sim * sim, HGF * hgf)
{
...
void Scenes::stepSquareFilm(double dt, Sim * sim, HGF * hgf)
{
    
}

void Scenes::stepTwistedFilm90(double dt, Sim * sim, HGF * hgf)
{
...
void Scenes::move_vertices(double dt, Sim *sim, HGF *hgf){
    using namespace Eigen;
...
void Scenes::wall_collision(double dt, Sim *sim, HGF *hgf){
    
    const size_t num_v=hgf->mesh().nv();
    for(size_t vi=0;vi<num_v;++vi){
     
        if(hgf->surfTrack()->pm_positions[vi][2]<hgf->bottom_level){
            hgf->surfTrack()->pm_positions[vi][2]=hgf->bottom_level;
            hgf->vel(vi)[2]=0.0;
        }
    }
...
void Scenes::add_circular_velocity(double dt, Sim *sim, HGF *hgf){
    using namespace Eigen;
    ...
    if(sim->m_time<1.0){
        // Add clockwise circular velocity field in the northern hemisphere.
        for(size_t ti=0; ti<num_t; ti++){
            ...
            if((y_lower <= tc.y()) && (tc.y() <= y_upper)) {
                ...
                hgf->triangle_velocities[ ti ] += sign * coef * tn.cross(y_axis);
                hgf->triangle_velocities[ ti ] -= hgf->triangle_velocities[ ti ].dot( tn ) / tn.squaredNorm()*tn;
            }
        }
    }
    
    if(sim->m_time>=5.0 and sim->m_time<6.0){
        // Add counter clockwise circular velocity field in the southern hemisphere.
        for(size_t ti=0; ti<num_t; ti++){
            const double y_upper = -0.5;
            const double y_lower = -0.8;
            const int sign = -1;
            const auto tc = hgf->triangle_centers[ti];
            if((y_lower <= tc.y()) && (tc.y() <= y_upper)) {
                const Vector3d tn = hgf->triangle_normals[ti];
...
void cylinder(const Eigen::Vector3d from, const Eigen::Vector3d till, Eigen::MatrixXd &V_cyl,Eigen::MatrixXi &F_cyl, const  int axis_devisions=10, const int height_devisions=5, const double eps=0.01){
    using namespace Eigen;

    Vector3d directed_edge=till-from;
    
    const double height=directed_edge.norm();
    igl_cylinder_arranged(height,axis_devisions, height_devisions,V_cyl,F_cyl, eps);
...
HGF * Scenes::sceneCylinder( Sim * sim, std::vector<LosTopos::Vec3d> & vs, std::vector<LosTopos::Vec3st> & fs, std::vector<LosTopos::Vec2i> & ls, std::vector<size_t> & cv, std::vector<Vec3d> & cx )
{
    using namespace Eigen;
    ...
    double height=1.0;
    double radius=10.0;
    Vector3d from(0,0,-height/2.0);
    Vector3d till(0,0,height/2.0);
	const int axis_devisions = N;
    const int height_devisions= N;
	cylinder(from,till,V,F, axis_devisions,height_devisions,0.0 );
    V.col(0)*=radius;
    V.col(1)*=radius;
    
    MatrixXi Ftemp=F;
    F<<Ftemp.col(0), Ftemp.col(2), Ftemp.col(1);
...
void Scenes::addCylindricalVelocity(double dt, Sim *sim, HGF *hgf){
    ...
    for(size_t vi=0;vi<num_v;++vi){
        ...
        Vector3d pos2d(hgf->pos(vi)[0],hgf->pos(vi)[1],0);
        double velScale=1e0;
        Vector3d additionalVelocity=velScale*pos2d.cross(Vector3d(0,0,1));
        
        hgf->fv(vi)+=additionalVelocity;
    }
    
}

void Scenes::addVerticalVelocity(double dt, Sim * sim, HGF * hgf){
    using namespace Eigen;
    size_t num_v = hgf->mesh().nv();
    const double velocity_coef=Options::doubleValue("vertical-velocity-coef");
    for( size_t vi = 0; vi < num_v; ++vi ){
        hgf->vel(vi)+=Vector3d(0,0,-velocity_coef);
    }
    
}

void Scenes::addThicknessForGradientTest( double dt, Sim * sim, HGF * hgf )
{
    using namespace Eigen;
    size_t num_v = hgf->mesh().nv();
    const double thickness_scale=0.5;
    const double thickness_band=thickness_scale*(hgf->max_th_nm-hgf->min_th_nm);
    
    size_t targetAxis=2;
    MatrixXd V;
    MatrixXi F;
    hgf->meshAsMatrices(V, F);
    
    const double maxPosValue=V.col(targetAxis).maxCoeff();
    const double minPosValue=V.col(targetAxis).minCoeff();
    
    const double avePosValue=(maxPosValue+minPosValue)/2.0;
    const double posBand=(maxPosValue-minPosValue);
    
    const size_t partition = 5;
    double thicknessValues[partition];
    for(size_t p=0;p<partition;++p){
        thicknessValues[p]=thickness_band*p;
    }
    
    for( size_t vi = 0; vi < num_v; ++vi ){
         hgf->thv(vi)+=(hgf->surfTrack()->pm_positions[vi][targetAxis]-avePosValue)/posBand * thickness_band;
        
    }
    
    hgf->computeLiquidVolumes(hgf->initialLiquidVolumes);
    
}

void Scenes::addThicknessByPerlinNoise( double dt, Sim * sim, HGF * hgf, int dimension,double frequency , std::uint32_t seed,  int octaves)
{
    using namespace Eigen;
    size_t num_v = hgf->mesh().nv();
    const double thickness_scale=0.5;
    ...
    const Vector3d maxPosValues(V.col(0).maxCoeff(),V.col(1).maxCoeff(),V.col(2).maxCoeff());

    const Vector3d minPosValues(V.col(0).minCoeff(),V.col(1).minCoeff(),V.col(2).minCoeff());
    
    const Vector3d avePosValues= (maxPosValues+minPosValues)/2.0;

    const Vector3d posBands=maxPosValues-minPosValues;

    dimension = HGF::clamp(dimension, 1,3);

    frequency = HGF::clamp(frequency, 0.1, 64.0);

    octaves = HGF::clamp(octaves, 1, 16);

    enum NoiseType{
        Standard,
        Octave
    };
    NoiseType noiseType= octaves==0? Standard:Octave;
    
    // Generate noise
    const siv::PerlinNoise perlin(seed);
    auto generateNoise=[&perlin,octaves,noiseType]( auto... args){
        switch(noiseType){
            case Standard:
                return perlin.noise(args...);
                break;
            case Octave:
                return perlin.octaveNoise(args...,octaves);
                break;
        }
    };
    ...
    for( size_t vi = 0; vi < num_v; ++vi ){
        ...
        for(size_t dim=0;dim<3;++dim){
            inputVec[dim]=frequency*(hgf->surfTrack()->pm_positions[vi][dim]-minPosValues[dim])/posBands[dim];
        }

        double noiseValue;
        switch (dimension) {
            case 1:
                noiseValue=generateNoise(inputVec[2]);
                break;
            case 2:
                noiseValue=generateNoise(inputVec[0],inputVec[1]);
                break;
            case 3:
                noiseValue=generateNoise(inputVec[0],inputVec[1],inputVec[2]);
                break;
        }

        hgf->thv(vi)+=noiseValue*thickness_band;

        hgf->thv(vi)=HGF::clamp(hgf->thv(vi),hgf->min_th_nm,hgf->max_th_nm);
    }
...
void Scenes::rotVectorField( double dt, Sim * sim, HGF * hgf ){
    // Make triangle velocity a rotation-only field.
    using namespace Eigen;
    // Compute rot vector of vertices.
    MatrixXd Rot_Vert;
    MatrixXd TriVelocity=hgf->getTriVelocity();
    hgf->computeRotationOnVertices(TriVelocity,Rot_Vert);
    
    // Convert this vector field to triangle vector field.
    hgf->vectorVert2tri(Rot_Vert, TriVelocity,true);
    // Set vertex and triangle velocity.
    hgf->setfv(Rot_Vert);
    hgf->setTriVelocity(TriVelocity);
}

void Scenes::curlNoise( double dt, Sim * sim, HGF * hgf,double frequency, std::uint32_t seed,  int octaves ){
    using namespace Eigen;
    ...
    NoiseType noiseType=octaves==0? Standard: Octave; // decide if octaves is zero or not.

    octaves = HGF::clamp(octaves, 1, 16);
    frequency = HGF::clamp(frequency, 0.1, 64.0);
    
    size_t num_v = hgf->mesh().nv();
    const double thickness_scale=0.3;
    const double thickness_band=thickness_scale*(hgf->max_th_nm-hgf->min_th_nm);
    
    // Set up inputs within 0 to 1
    MatrixXd V;
    MatrixXi F;
    hgf->meshAsMatrices(V, F);
    const Vector3d maxPosValues(V.col(0).maxCoeff(),V.col(1).maxCoeff(),V.col(2).maxCoeff());
    ...
    for( size_t vi = 0; vi < num_v; ++vi ){
        ...
        double outX=generateNoise(inputVec[0],inputVec[1],inputVec[2]);
        double outY=generateNoise(inputVec[1],inputVec[2],inputVec[0]);
        double outZ=generateNoise(inputVec[2],inputVec[0],inputVec[1]);
        
        const double coef=1.0*hgf->ext_force;
        hgf->fv(vi)+=coef*Vector3d(outX,outY,outZ);
        hgf->projectToTangent(hgf->fv(vi),hgf->vertex_normals[vi],false);
        
    }
    MatrixXd CurlTrivelocity;
    hgf->vectorVert2tri(hgf->fvmat(),CurlTrivelocity);
    hgf->setTriVelocity(CurlTrivelocity + hgf->getTriVelocity());
    hgf->project_tri(dt, hgf->equation);
